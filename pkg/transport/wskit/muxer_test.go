...
func TestShouldGetOrCreateStreamRejectsWhenStreamLimitExceeded(t *testing.T) {
	...
	assert.ErrorIs(t, m.tombstones[msg.ChannelID].reason, ErrTooManyStreams)
}
...
func TestShouldReturnMuxerDiagnosticsSnapshot(t *testing.T) {
	...
	m.tombstones[uuid.New()] = muxerTombstone{reason: ErrMuxerClosed, sequence: 1}
...
func TestShouldGetOrCreateStreamIgnoresTombstonedChannel(t *testing.T) {
	...
	m := &WebSocketMuxer{
		...
		name:        "server",
		channels:    make(map[uuid.UUID]*MuxerBidiStream),
		tombstones:  map[uuid.UUID]muxerTombstone{channelID: {reason: ErrStreamOverloaded, sequence: 1}},
		done:        make(chan struct{}),
		nodeManager: manager,
		session:     &muxerSession{allowAll: true},
	}
...
func TestShouldBoundMuxerTombstones(t *testing.T) {
	...
	_, oldestRetained := m.tombstones[channelIDs[0]]
	_, newestRetained := m.tombstones[channelIDs[len(channelIDs)-1]]
	assert.False(t, oldestRetained)
	assert.True(t, newestRetained)
	assert.ErrorIs(t, m.tombstones[channelIDs[len(channelIDs)-1]].reason, ErrMuxerClosed)
}
